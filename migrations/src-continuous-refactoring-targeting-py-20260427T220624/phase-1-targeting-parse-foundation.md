...
2. Keep `resolve_targets()` as the public entrypoint and add the helper without changing current loop callsites:
...
- Existing `loop.py` path parsing may still be present; phase 2 owns loop delegation and removal.

## Definition of Done
- `targeting.py` has a first-class path parser ready for loop delegation.
...
- No edits are made in `loop.py` during this phase.
