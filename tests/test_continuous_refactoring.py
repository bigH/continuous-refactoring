...
def test_package_exports_are_stable() -> None:
    exports = continuous_refactoring.__all__

    assert isinstance(exports, tuple)
    assert exports[: len(PACKAGE_SURFACE_EXPORTS)] == PACKAGE_SURFACE_EXPORTS
    assert len(exports) == len(set(exports))


def test_package_exports_contain_known_public_symbols() -> None:
    for name in (
        "AttemptStats",
        "build_command",
        "run_observed_command",
...
def test_migration_manifest_codec_stays_internal() -> None:
    ...
    assert not set(migration_manifest_codec.__all__) & set(continuous_refactoring.__all__)
    assert "migration_manifest_codec" not in continuous_refactoring.__all__
